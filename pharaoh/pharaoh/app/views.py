...
from pharaoh.app.models import Sentence, User, File, MyError, LockError, get_languages, get_fileIDs, get_files_for_page, find_file, get_sentences_in_file, get_file_paths
...
def file_browser(language, username, page):
    ...
    '''
    files = get_fileIDs('en', language)
    num_files = files.count()
    page_files = get_files_for_page(page, app.config['NUM_FILES_PER_PAGE'], files)
...
def file_editor(file, language, username):
    ...
    if file[-1] == '/':
        file = file[:-1]
    f = File(oid=find_file("en", language, file)['_id'])
    u = User(username=username)
    if f.grab_lock(u._id) is False:
        return redirect('/edit/{0}/{1}/{2}/423'.format(username, language, file))

    sentences = get_sentences_in_file(urllib.url2pathname(file), 'en', language)
...
def edit_translation():
    ...
    try:
        ...
        t = Sentence(oid=j[u'old'][u'_id'])
        editor = User(username=j[u'new'][u'editor'])
...
def approve_translation():
    ...
    j = fix_json(request.json)
    t = Sentence(oid=j[u'old'][u'_id'])
    approver = User(username=j[u'new'][u'approver'])
    try:
        t.approve(approver)
...
def unapprove_translation():
    ...
    j = fix_json(request.json)
    t = Sentence(oid=j[u'old'][u'_id'])
    unapprover = User(username=j[u'new'][u'unapprover'])
    try:
