...
class DictToCorpusTestCase(unittest.TestCase):
    ...
    def test_english(self):
        create_corpus_from_dictionary(TEST_PATH+"/test_files/dict.txt", TEST_PATH+"/temp_files/source.txt", TEST_PATH+"/temp_files/target.txt")
        with open(TEST_PATH+"/temp_files/source.txt") as f:
            self.assertEqual(f.read().strip(), "April\nApr\nAugust\nAug")

    def test_spanish(self):
        create_corpus_from_dictionary(TEST_PATH+"/test_files/dict.txt", TEST_PATH+"/temp_files/source.txt", TEST_PATH+"/temp_files/target.txt")
        with open(TEST_PATH+"/temp_files/target.txt") as f:
...
class PoToCorpusTestCase(unittest.TestCase):
    ...
    def test_english(self):
        create_corpus_from_po(TEST_PATH+"/test_files/about.po", TEST_PATH+"/temp_files/source.txt", TEST_PATH+"/temp_files/target.txt")
        with open(TEST_PATH+"/temp_files/source.txt") as f:
...
class OneCorpusTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conf_file = TEST_PATH+"/configs_test/one_corpus.yaml"

    def setUp(self):
        self.cconf = ingest_yaml_doc(self.conf_file)
        for source in self.cconf['sources']:
            source['source_file_path'] = TEST_PATH + '/'+ source['source_file_path']
            source['target_file_path'] = TEST_PATH + '/'+ source['target_file_path']
        self.cconf['container_path'] = TEST_PATH + '/'+ self.cconf['container_path']
    ...
    def test_spanish_train(self):
        create_hybrid_corpora(self.cconf)
        with open(TEST_PATH+"/temp_files/train.en-es.es") as f:
            self.assertEqual(f.read().strip(), "hola")

    def test_english_tune(self):
        create_hybrid_corpora(self.cconf)
        with open(TEST_PATH+"/temp_files/tune.en-es.en") as f:
            self.assertEqual(f.read().strip(), "")

    def test_spanish_tune(self):
        create_hybrid_corpora(self.cconf)
        with open(TEST_PATH+"/temp_files/tune.en-es.es") as f:
            self.assertEqual(f.read().strip(), "")

    def test_english_test(self):
        create_hybrid_corpora(self.cconf)
        with open(TEST_PATH+"/temp_files/test.en-es.en") as f:
            self.assertEqual(f.read().strip(), "")

    def test_spanish_test(self):
        create_hybrid_corpora(self.cconf)
        with open(TEST_PATH+"/temp_files/test.en-es.es") as f:
            self.assertEqual(f.read().strip(), "")
...
class EvenCorpusTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conf_file = TEST_PATH+"/configs_test/even_corpora.yaml"
...
class HiddenCorpusTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.conf_file = TEST_PATH+"/configs_test/hidden_corpora.yaml"
