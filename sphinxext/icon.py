...
class IconBrandNode(nodes.General, nodes.Inline, nodes.Element):
    @staticmethod
    def visit_icon(self, node):
        # I've tested this in Safari w/ macOS Voiceover
        self.body.append(
            self.starttag(
                node,
                'span',
                CLASS='fab fa-{}'.format(node['css_class']),
                **{'title': node['label']}))
        self.body.append(self.starttag(node, 'span', CLASS='screenreader'))
        self.body.append(node['label'])
...
def icon_brand_role(typ, rawtext, text, lineno, inliner, options={}, content=[]):
    errors = []
    has_explicit_title, label, css_class = split_explicit_title(text)
    if css_class not in ICONSB:
        errors.append(inliner.reporter.error('Unknown icon: ' + css_class, line=lineno))
    ...
    config = inliner.document.settings.env.config
    if config._raw_config['tags'].eval_condition('html'):
        return [IconBrandNode(css_class=css_class, label=label)], []
...
def setup(app):
    ...
    app.add_node(IconBrandNode, html=(IconBrandNode.visit_icon, IconBrandNode.depart_icon))
    roles.register_local_role('iconb', icon_brand_role)
