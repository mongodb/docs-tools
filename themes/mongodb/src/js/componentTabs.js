...
let tabSets = {};
let anonymousTabIndex = 0;
class TabSet {
    constructor(tabType, tabStripElements, tabContents) {
        this.type = tabType;
    ...
    update(tabId, isUserAction) {
        ...
        if (!tabId) {
            const tabPref = getTabPref();
            if (!this.type && tabPref.pages && tabPref.pages[window.location.pathname]) {
                // Check if current page has a one-off page specific pref
                tabId = tabPref.pages[window.location.pathname];
            } else if (this.type && tabPref[this.type]) {
    ...
    static register(tabElement) {
        ...
        const tabType = tabElement.getAttribute('data-tab-preference');
        ...
        if (tabType) {
            ...
            const tabSet = new TabSet(tabType, tabStripElements, [tabContent], false);
            tabSets[tabType] = tabSet;
            return;
        }

        const tabSet = new TabSet(anonymousTabIndex, tabStripElements, [tabContent]);
        tabSets[anonymousTabIndex] = tabSet;
        anonymousTabIndex += 1;
        return;
    }
}
